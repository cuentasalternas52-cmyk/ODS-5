...
app.post("/chat", async (req, res) => {
  try {
    ...
    if (!process.env.OPENAI_API_KEY) return res.json({ reply: "IA no configurada" });
    ...
    const reply = data?.output?.[0]?.content?.[0]?.text || "No se pudo generar respuesta";
    res.json({ reply });
  } catch (error) {
